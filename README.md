...
Mở thư mục C:\xampp\htdocs trên máy tính, chuột phải chọn "Git Bash Here" và gõ lệnh:

git clone https://github.com/dotrunghieuwork/Shoppe.git ShopBTL
...
2. Mở file database/BTLDatabaseMe.sql -> Bấm nút Tia sét để tạo cấu trúc bảng.
3. Mở file database/insertSampleData.sql -> Bấm nút Tia sét để nạp dữ liệu mẫu.
4. Quan trọng: Vào file server/config/db.php trong VS Code, sửa lại dòng $pass thành mật khẩu MySQL của máy bạn (ví dụ: 123456, admin hoặc để trống).

---

## 3. Cách chạy dự án để Code (Hằng ngày)
...
- LƯU Ý: KHÔNG bấm Start dòng MySQL trong XAMPP (để tránh xung đột cổng 3306 với MySQL Workbench). Chúng ta dùng MySQL của Workbench.
...
2. Code xong, kiểm tra lại rồi mới đẩy lên:
git add .
git commit -m "Ten ban - Mo ta ngan gon viec da lam"
git push origin main

3. Nếu gặp lỗi Conflict: Tuyệt đối không xóa lung tung. Hãy chụp ảnh gửi lên nhóm hoặc gọi Leader để cùng xử lý.
